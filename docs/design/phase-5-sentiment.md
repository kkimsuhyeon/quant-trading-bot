...
## 결과 (BTC/ETH 4h, F&G 3068일(2018~), 극탐욕 ≥75 = 전체의 10.8% 날)

baseline(`use_sentiment=False`) vs +sentiment(극탐욕 회피). Δ는 (+sent − base), **MDD Δ가 +면 낙폭 얕아짐(개선)**.

| 자산 | 전략 | 구성 | Return% | MDD% | Sharpe | #Tr |
|---|---|---|---|---|---|---|
| BTC | Regime | baseline | +202.5 | -34.9 | 0.56 | 143 |
| | | **+sentiment** | +189.0 | **-33.5** | **0.62** | 173 |
| BTC | Keltner | baseline | +83.1 | -37.3 | 0.46 | 116 |
| | | **+sentiment** | +91.7 | **-29.6** | **0.57** | 113 |
| ETH | Regime | baseline | +104.8 | -42.9 | 0.27 | 142 |
| | | **+sentiment** | +99.6 | -42.9 | 0.29 | 172 |
| ETH | Keltner | baseline | +103.3 | -28.7 | 0.45 | 112 |
| | | **+sentiment** | +104.7 | -27.7 | 0.49 | 109 |

OOS(뒤 30%): BTC Regime -18.1→-15.8(Sharpe -0.60→-0.51), BTC Keltner -30.0→-25.6(-1.46→-1.29), ETH Regime +42.4→+40.3(0.46→0.44), ETH Keltner +5.6→+12.7(0.11→0.25).

**관찰:** 8개 칸 중 **Sharpe는 7개에서↑**(유일한 예외 ETH Regime OOS −0.02), **MDD는 악화한 칸이 없다**(위 표의 in-sample 4칸은 개선/유지, OOS 4칸도 별도 확인 시 악화 없음 — 표엔 OOS Return/Sharpe만 기재). 즉 필터는 **해롭지 않고 약하게 일관 도움**(예상했던 "추세 수익 깎임"이 크지 않았다 — 극탐욕 회피가 추세를 크게 자르지 않음).

## 판정 (사전등록 기준 대조)
- **Keltner: 명확 개선.** BTC MDD -37.3→-29.6 (**상대 ~21%↓, 10% 바 통과**), Sharpe 0.46→0.57, Return도 +8.6. ETH도 소폭 개선. → 기준 충족.
- **Regime(1순위): 개선했으나 미흡.** BTC MDD -34.9→-33.5 (**상대 ~4% < 10% 바**), Sharpe +0.06, Return 93% 보존. 악화는 아니나 **사전 MDD 바를 못 넘음.**
- 사전등록 해석표상 **"Keltner만 명확 개선 → 전략 특이 효과로 낮춰 해석"**에 해당. **게임체인저 아님.**

**결론:** F&G 극탐욕 회피는 **약하지만 일관된 리스크오프 효과**(Sharpe 7/8↑, MDD 악화 0)를 보였고, **Keltner에서만 사전 바를 명확히 통과**했다. 1순위 Regime은 미흡. → **"검증된 보편 오버레이"로 승격하진 않는다.** 단 *해롭지 않고 약한 양(+)*이라, Keltner 계열에 한해 약한 보조 후보로 메모. 추세전략에 거품회피가 큰 손해를 안 준다는 것(예상과 달리)은 의미 있는 관찰.

## ⚠️ 정직한 한계
- **효과가 작다.** Regime은 사전 바 미달, Keltner만 통과 = 전략 특이일 수 있음(단일 역사 경로). "심리가 가격에 새 정보를 *크게* 더하진 못한다"에 가깝다.
- turnover 일부↑(Regime 143→173) — 수수료 0.1% 반영했으나 실제 더 들 수 있음.
- F&G는 BTC/전체시장 지수(자산별 아님). ETH에도 같은 지수 사용 = 근사.
- 단일 임계(75)·단일 방향(거품회피)만 봄. (80 등 sweep 금지 — 했으면 스누핑.)
