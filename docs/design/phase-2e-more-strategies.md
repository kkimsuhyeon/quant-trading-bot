...
→ 검증 대상: **Keltner, Regime**. **Z-score MR은 탈락**(B&H 낙폭 방어 실패 — 예상대로 MR은 손절과 충돌, 가드레일대로 손절 통일 적용한 결과 탈락. *정상*).

### B. robustness 3축 — Keltner 돌파
| | OOS 방어 | 파라미터 민감도(16조합) | 워크포워드 | FULL |
|---|---|---|---|---|
| BTC | ✅ (OOS MDD -34.1 vs B&H -52.7) | ✅ 100% 방어·100% 양수 | ✅ 5/5 | +83.1% / -37.3% |
| ETH | ✅ (OOS +5.6%·MDD -27.7 vs -68.0) | ✅ 100% 방어·100% 양수 | ✅ 5/5 | +103.3% / -28.7% |

### C. robustness 3축 — Regime 필터
| | OOS 방어 | 파라미터 민감도(sma_n 5값) | 워크포워드 | FULL |
|---|---|---|---|---|
| BTC | ✅ (OOS MDD -31.8 vs -52.7) | ✅ 100% 방어(sma 100~300 양수) | ✅ 5/5 | +202.5% / -34.9% |
| ETH | ✅ (OOS +42.4%·MDD -41.1 vs -68.0) | ✅ 100% 방어 | ✅ 4/5 | +104.8% / -42.9% |

## 판정
- ✅ **Keltner 돌파 — 검증 통과 (BTC+ETH).** 지금까지 **최강 후보** — ETH OOS도 양수, 파라미터 전 영역 양수+방어. 변동성 밴드 돌파 + EMA 빠른 청산의 조합이 견고.
- ✅ **Regime 필터 — 검증 통과 (BTC+ETH).** **B&H를 수익·낙폭 모두 이긴 첫 전략**(BTC +202% vs +75%). 가장 단순(가격 > SMA200이면 보유).
- 🔴 **Z-score MR — 탈락** (탐색서 방어 실패). MR이 크립토 4h에서 안 된다는 걸 또 확인.

## 정직한 한계 (★특히 중요 — 과신 금지)
- **이들이 B&H를 이긴 핵심 이유 = 이 5년에 2022 대폭락이 들어있어서다.** 이들의 엣지는 "큰 낙폭을
  피하는 것"이라, **폭락이 있는 창에선 빛나고 없는 창에선 B&H에 뒤진다.** 실제로 **OOS(최근 18개월,
  큰 폭락 없음)의 절대수익은 BTC에서 음수**(Keltner -30%, Regime -18%)다 — "폭락 없으면 B&H 못 이긴다"를
  스스로 보여줌. 즉 화려한 총수익에도 불구하고 본질은 여전히 **"낙폭 방어 도구"**다. **+202%를 미래
  기대수익으로 읽으면 안 된다.**
- **Keltner·Regime·Donchian·SMA+stop 4개 다 추세/돌파 계열 = 서로 상관 높음.** 검증 후보가 4개여도
  **분산된 포트폴리오가 아니다**(같이 오르고 같이 깨짐). 진짜 분산은 다른 엣지(선물 캐리·횡단모멘텀, Phase 7) 필요.
- 2자산·단일 역사 경로 한계 여전(둘 다 크립토 베타).

## 누적 검증 통과 후보 (전부 추세 계열, 상관 높음)
1. **Keltner 돌파 (4h)** — 최강(ETH OOS 양수)
2. **Regime 필터 (4h)** — B&H 총수익도 이김(단 regime 의존)
3. Donchian 돌파 (4h)
4. SMA+손절 (4h)

## 다음 단계
- Phase 4 페이퍼 후보가 4개로 늘었다(상관 높으니 1~2개 대표 선정 가능).
- Z-score MR은 폐기. MR 계열은 크립토 4h에서 일관되게 실패 → 추가 MR 탐색 비권장.
